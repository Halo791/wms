                ...
                {approvals.map(a => (
                  <tr key={a.id}>
                    ...
                    <td className="text-right">
                      <button className="icon-btn" style={{color: 'var(--success)', background: 'var(--success-transparent)', marginRight: '0.5rem'}} onClick={() => openProcessModal(a, 'APPROVED')} title="Approve"><CheckCircle2 size={18}/></button>
                      <button className="icon-btn" style={{color: 'var(--danger)', background: 'var(--danger-transparent)'}} onClick={() => openProcessModal(a, 'REJECTED')} title="Reject"><XCircle size={18}/></button>
                    </td>
                  </tr>
                ))}
                {approvals.length === 0 && (
                  <tr>
                    <td colSpan="7">
                      <div className="table-empty-state">
                        <ShieldCheck />
                        <div className="table-state-title">Tidak ada antrean approval</div>
                        <div className="table-state-desc">Semua transaksi gudang sudah diotorisasi. Ruang kerja bersih! ✨</div>
                      </div>
                    </td>
                  </tr>
                )}
              ...
              <tbody>
                {logs.map(l => (
                  <tr key={l.id}>
                    ...
                    <td><span className="badge" style={{background: 'var(--primary-transparent)', color: 'var(--primary)'}}>{l.action}</span></td>
                ...
                {logs.length === 0 && (
                  <tr>
                    <td colSpan="6">
                      <div className="table-empty-state">
                        <Activity />
                        <div className="table-state-title">Belum ada log aktivitas</div>
                        <div className="table-state-desc">Aktivitas pengguna di dalam sistem akan tercatat di sini.</div>
                      </div>
                    </td>
                  </tr>
                )}
          ...
          <div className="modal-content glass small">
            ...
            <form onSubmit={submitProcess} style={{ marginTop: '1rem' }}>
              ...
              <div className="modal-footer" style={{ marginTop: '1.5rem' }}>
                ...
                <button type="submit" className="primary-btn" style={{ background: approvalAction === 'APPROVED' ? 'var(--success)' : 'var(--danger)' }}>{approvalAction === 'APPROVED' ? 'Ya, Approve' : 'Tolak Transaksi'}</button>
              </div>
            </form>
          </div>
        </div>
